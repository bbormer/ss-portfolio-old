      ...
      @csrf
        <div>
          ...
          <div class="form-group mb-6 {{ $errors->has('name') ? 'has-error' : '' }}">
              ...
              <textarea name="content" placeholder=" {{ __('message (required)') }}" class="form-control w-[95%] lg:w-[600px] rounded border-green-300 border-2 p-2 mx-auto lg:mx-0" rows="5">{{ old('content') }}</textarea>
              @error('content')
